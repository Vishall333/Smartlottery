...
class PaymentProcessor {
  ...
  async processPaymentVerifications() {
    ...
    try {
      ...
      for (const paymentDoc of pendingPaymentsSnapshot.docs) {
        ...
        // For QR/UPI payments, NEVER auto-credit - require admin verification ONLY
        if (paymentData.method && ['gpay', 'phonepe', 'paytm'].includes(paymentData.method)) {
          // QR payments ONLY credit if ADMIN has explicitly verified (no time-based auto-crediting)
          if (paymentData.adminVerified === true && paymentData.adminVerifiedAt) {
            await this.creditUserBalance(paymentData, batch);
            
            batch.update(paymentDoc.ref, {
              status: 'completed',
              completedAt: admin.firestore.FieldValue.serverTimestamp(),
              verificationMethod: 'admin_manual_verification_only'
            });

            processedCount++;
          }
          // SKIP all auto-processing for QR payments - admin verification required
          continue;
        } else {
  ...
  async recordPayment(userId, amount, method, paymentId) {
    ...
    try {
      const isQRPayment = ['gpay', 'phonepe', 'paytm'].includes(method);
      const paymentRecord = {
        userId: userId,
        amount: amount,
        method: method,
        paymentId: paymentId,
        status: isQRPayment ? 'pending_admin_verification' : 'pending_verification',
        createdAt: Date.now(),
        timestamp: admin.firestore.FieldValue.serverTimestamp(),
        manuallyConfirmed: false,
        adminVerified: false,
        requiresAdminApproval: isQRPayment,
        isQRPayment: isQRPayment,
        autoCredit: !isQRPayment // Only non-QR payments can auto-credit
      };
...
app.post('/api/confirm-payment', async (req, res) => {
  try {
    ...
    // For QR payments, BLOCK manual confirmation completely - admin verification ONLY
    if (['gpay', 'phonepe', 'paytm'].includes(paymentData.method)) {
      return res.status(403).json({
        success: false,
        message: 'QR payments cannot be manually confirmed. Admin verification required. Contact support.',
...
app.post('/api/admin/verify-payment', async (req, res) => {
  try {
    ...
    if (verified) {
      // Admin approved - credit the user ONLY after admin verification
      const batch = db.batch();
      await paymentProcessor.creditUserBalance(paymentData, batch);
      ...
      batch.update(paymentDoc.ref, {
        ...
        completedAt: admin.firestore.FieldValue.serverTimestamp(),
        verificationMethod: 'admin_verified_only',
        creditedBy: 'admin'
      });

      await batch.commit();

      console.log(`✅ Admin verified and credited ₹${paymentData.amount} to user ${paymentData.userId}`);

      res.json({
        success: true,
        message: 'Payment verified and credited successfully by admin'
      });
    } else {
      // Admin rejected
...
app.get('/api/payment-status/:recordId', async (req, res) => {
  try {
    ...
    res.json({
      ...
      isQRPayment: paymentData.isQRPayment || false,
      adminVerified: paymentData.adminVerified || false,
      autoCredit: paymentData.autoCredit !== false
    });
